...
def copy_file_to_target_folder(full_name_file, target_folder, overwrite=True):
    full_target_name = os.path.basename(full_name_file)
    if overwrite or (not overwrite and not os.path.exists(full_name_file)):
        shutil.copy(full_name_file, os.path.join(target_folder, full_target_name))


def copy_files_from_list(list_file, target_folder):
    with open(list_file, "r") as list_f:
        for full_name_file in list_f:
            copy_file_to_target_folder(str(full_name_file).strip(), target_folder)
            copy_file_to_target_folder(str(full_name_file).strip() + ".sgy", target_folder)
